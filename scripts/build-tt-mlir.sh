...
if [ -z "$LLVM_BUILD_DIR" ]; then
    echo "LLVM_BUILD_DIR must be set to the triton LLVM build root directory (typically in ~/.triton/llvm/llvm-OSDISTRO-ARCH)"
    exit 1
fi
...
TTMLIR_VENV_DIR=$TRITON_VENV_DIR
...
MLIR_INCLUDE_DIRS=$LLVM_BUILD_DIR/include
...
if [ -z "$NO_TTMLIR_RUNTIME" ]; then
    ...
    cmake -G Ninja -B build -DTTMLIR_ENABLE_BINDINGS_PYTHON=OFF -DTTMLIR_ENABLE_RUNTIME=ON -DTT_RUNTIME_ENABLE_TTNN=ON -DTT_RUNTIME_ENABLE_TTMETAL=ON -DTTMLIR_ENABLE_RUNTIME_TESTS=ON
    ...
    cmake -G Ninja -B build -DTTMLIR_ENABLE_BINDINGS_PYTHON=OFF -DTTMLIR_ENABLE_RUNTIME=OFF -DTT_RUNTIME_ENABLE_TTNN=OFF -DTT_RUNTIME_ENABLE_TTMETAL=OFF -DTTMLIR_ENABLE_RUNTIME_TESTS=OFF
    cmake --build build
fi
