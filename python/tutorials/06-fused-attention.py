...
configs = [
    ...
    for w in ([1] if is_cpu() else [4, 8])\
]
if "PYTEST_VERSION" in os.environ:
    # Use a single config in testing for reproducibility
    configs = [
        triton.Config(dict(BLOCK_M=64, BLOCK_N=64), num_stages=1, num_warps=1, pre_hook=_host_descriptor_pre_hook),
    ]
...
@pytest.mark.parametrize("H", [2, 24])  # 2, 48
@pytest.mark.parametrize("N_CTX", [128])  #, 1024, (2 if is_hip() or is_cpu() else 4) * 1024])
