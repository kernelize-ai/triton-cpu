...
static LogicalResult offsetPidByBlockIndex(triton::FuncOp funcOp, unsigned blockIdxArgPos) {
  ...
  assert(programIdOps.size() == 1 && "expected exactly one tt.get_program_id op");

OpBuilder b(programIdOps[0]->getNextNode());
  Value blockOffset = b.create<arith::AddIOp>(programIdOps[0]->getLoc(), programIdOps[0].getResult(), blockIdx);
  programIdOps[0].getResult().replaceUsesWithIf(blockOffset , [&](OpOperand &operand) {
...
static triton::FuncOp
cloneTTFuncWithExtraI32Arg(ModuleOp mod,
                           triton::FuncOp src,
                           StringRef newName) {
  ...
  SmallVector<Type> newInputs(oldFTy.getInputs().begin(), oldFTy.getInputs().end());
  Type i32Ty = IntegerType::get(ctx, 32);
  newInputs.push_back(i32Ty);
  auto newFTy = FunctionType::get(ctx, newInputs, oldFTy.getResults());

  // Copy user attrs (excluding structural), arg attrs (+ empty for new arg), res attrs.
  SmallVector<NamedAttribute> userAttrs = collectClonableOpAttrs(src, /*excludeNoInline=*/true);
  ...
  auto newFunc = b.create<triton::FuncOp>(
      src.getLoc(), newName, newFTy,
      /*attrs=*/userAttrs,
      /*argAttrs=*/argDicts);
  ...
  if (auto resArr = src->getAttr(kAttrResAttrs))
    ...
    for (auto it : llvm::zip(oldEntry.getArguments(),
...
static triton::FuncOp buildWrapper(ModuleOp mod, triton::FuncOp kernel, triton::FuncOp impl, StringRef name) {
      MLIRContext *ctx = mod.getContext();
  OpBuilder b(mod.getBodyRegion());

    Type i32Ty = IntegerType::get(ctx, 32);
    // use the original kernel to avoid pulling in the extra block param (TODO we should just embed that in the getProgramId op)
  ...
  SmallVector<NamedAttribute> userAttrs = collectClonableOpAttrs(kernel);
   ...
   // Arg attrs: copy 1:1 for the first original args; add empty for the 3 new ones.
  SmallVector<DictionaryAttr> argDicts = getArgAttrArray(kernel);
  argDicts.resize(wrapInputs.size(), DictionaryAttr::get(ctx, {}));

  auto wrap = b.create<triton::FuncOp>(kernel.getLoc(), name, wrapTy,
  ...
  Value bStartIdx = wb.create<arith::IndexCastOp>(wrap.getLoc(), wb.getIndexType(), bStart);
  Value bEndIdx   = wb.create<arith::IndexCastOp>(wrap.getLoc(), wb.getIndexType(), bEnd);
  
    Value bStepIdx = wb.create<arith::ConstantIndexOp>(wrap.getLoc(), 1); // TODO: should we make this a param too? 

   scf::ForOp forOp = wb.create<scf::ForOp>(wrap.getLoc(), bStartIdx, bEndIdx, bStepIdx, ValueRange{});
  {
    Block* body = forOp.getBody();
    OpBuilder fb(body, body->begin());

       Value bI32 = fb.create<arith::IndexCastOp>(wrap.getLoc(), i32Ty, forOp.getInductionVar());
    ...
    fb.create<triton::CallOp>(wrap.getLoc(), impl.getSymName(), TypeRange{}, callArgs);
  }
...
struct AddKernelStreamPass
  ...
  void runOnOperation() override {
    ...
    // 1. Clone the existing kernel, rename to `kernel`_impl, and add an i32 parameter which is the block index offset
#if 1
    StringRef oldName = kernel.getName();
    std::string implName = (oldName + ".impl").str();
    triton::FuncOp implFunc = cloneTTFuncWithExtraI32Arg(moduleOp, kernel, implName);
#else
    SmallVector<Type> argTys(kernel.getArgumentTypes().begin(), kernel.getArgumentTypes().end());
    Type i32Ty = IntegerType::get(ctx, 32);
    argTys.push_back(i32Ty); // block index 


    auto kernelImpl = b.create<triton::FuncOp>(kernel.getLoc(),
        StringRef(implName),
  ...
  for (auto it : llvm::zip(kernel.getArguments(),
                           kernelImpl.getBody().front().getArguments().take_front(kernel.getNumArguments())))
  ...
  for (Operation &op : kernel.getBody().front())
    ...
    // 2. Rewrite the tt.get_program_id operation to add the block index offset to the return value
    unsigned blockIdxOffset = implFunc.getNumArguments() - 1;
    if (failed(offsetPidByBlockIndex(implFunc, blockIdxOffset)))
      return signalPassFailure();

    // 3. Add the wrapper function calling kernel_impl in a loop over block_start to block_end offsets (kernel function parameters) 
